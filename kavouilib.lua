--[[ Custom Kavo Ui By 0lliware productions The Ui Used the Kavo The Ui Used The Fonts https://create.roblox.com/docs/reference/engine/enums/Font ]]--
...
mouse.Move:Connect(function()
	lastInputPos = Vector2.new(mouse.X, mouse.Y)
end)
...
local themeStyles = {
	OlliwareTheme = {
		SchemeColor = Color3.fromRGB(0, 120, 60),
		Background = Color3.fromRGB(12, 12, 12),
		Header = Color3.fromRGB(10, 10, 10),
		TextColor = Color3.fromRGB(230, 230, 230),
		ElementColor = Color3.fromRGB(18, 24, 20)
	},
	DarkTheme = {
...
function Kavo.CreateLib(kavName, themeList)
	...
	elseif themeList == "Twiware" then
		themeList = themeStyles.Twiware
	elseif themeList == "Serpent" then
		themeList = themeStyles.Serpent
	else
		if themeList.SchemeColor == nil then themeList.SchemeColor = Color3.fromRGB(74, 99, 135) end
		if themeList.Background == nil then themeList.Background = Color3.fromRGB(36, 37, 43) end
		if themeList.Header == nil then themeList.Header = Color3.fromRGB(28, 29, 34) end
		if themeList.TextColor == nil then themeList.TextColor = Color3.fromRGB(255,255,255) end
		if themeList.ElementColor == nil then themeList.ElementColor = Color3.fromRGB(32, 32, 38) end
	end
	...
	tabFrames.ScrollBarThickness = 6  -- улучшено для телефонов
	tabFrames.ScrollingDirection = Enum.ScrollingDirection.Y
	tabFrames.ElasticBehavior = Enum.ElasticBehavior.Always  -- плавный скролл
		...
		function Sections:NewSection(secName, hidden)
			...
			function Elements:NewButton(bname,tipINf, callback)
				...
				btn.MouseButton1Click:Connect(function()
					if not focusing then
						...
						c.Parent = btn
						local x, y = (lastInputPos.X - c.AbsolutePosition.X), (lastInputPos.Y - c.AbsolutePosition.Y) -- исправлено
			...
			function Elements:NewTextBox(tname, tTip, callback)
				...
				TextBox.FocusLost:Connect(function(EnterPressed)
					...
					if not EnterPressed then return else
				...
				btn.Activated:Connect(function()
					if not focusing then
						if toggled == false then
							game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear,Enum.EasingDirection.In), { ImageTransparency = 0 }):Play()
							local c = sample:Clone()
							c.Parent = btn
							local x, y = (lastInputPos.X - c.AbsolutePosition.X), (lastInputPos.Y - c.AbsolutePosition.Y)
							c.Position = UDim2.new(0, x, 0, y)
							local len, size = 0.35, nil
							if btn.AbsoluteSize.X >= btn.AbsoluteSize.Y then
								size = (btn.AbsoluteSize.X * 1.5)
							...
							game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear,Enum.EasingDirection.In), { ImageTransparency = 1 }):Play()
							local c = sample:Clone()
							c.Parent = btn
							local x, y = (lastInputPos.X - c.AbsolutePosition.X), (lastInputPos.Y - c.AbsolutePosition.Y)
							c.Position = UDim2.new(0, x, 0, y)
			...
			function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback)
				...
				local function updateSlider(posX)
					local relativeX = math.clamp(posX - sliderDrag.AbsolutePosition.X, 0, 149)
					local newValue = math.floor((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * relativeX) + tonumber(minvalue))
					val.Text = newValue
			...
			function Elements:NewDropdown(dropname, dropinf, list, callback)
				...
				moreInfo.ZIndex = 9
				moreInfo.RichText = true
				moreInfo.Font = Enum.Font.SourceSansLight
			...
			function Elements:NewColorPicker(colText, colInf, defcolor, callback)
				...
				local dragColor = false
				local dragDark = false
				...
				local function cp(inputPos)
					if colorpicker then
						local pos = inputPos or lastInputPos
						local x = pos.X - rgb.AbsolutePosition.X
						local y = pos.Y - rgb.AbsolutePosition.Y
					...
					if darknesss then
						local pos = inputPos or lastInputPos
						local y = pos.Y - dark.AbsolutePosition.Y
				...
				local function togglerainbow()
					if rainbow then
						game.TweenService:Create(toggleEnabled, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), { ImageTransparency = 1 }):Play()
						rainbow = false
						rainbowconnection:Disconnect()
					else
						game.TweenService:Create(toggleEnabled, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), { ImageTransparency = 0 }):Play()
						rainbow = true
				...
				-- Перетаскивание через UserInputService
				local function startDrag(input, type)
					if type == "color" then
						colorpicker = true
				...
				uis.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						if rgb.AbsolutePosition.X <= pos.X and pos.X <= rgb.AbsolutePosition.X + rgb.AbsoluteSize.X and
						   rgb.AbsolutePosition.Y <= pos.Y and pos.Y <= rgb.AbsolutePosition.Y + rgb.AbsoluteSize.Y then
							startDrag(input, "color")
						elseif darkness.AbsolutePosition.X <= pos.X and pos.X <= darkness.AbsolutePosition.X + darkness.AbsoluteSize.X and
							   darkness.AbsolutePosition.Y <= pos.Y and pos.Y <= darkness.AbsolutePosition.Y + darkness.AbsoluteSize.Y then
							startDrag(input, "dark")
						end
					end
				end)

				uis.InputChanged:Connect(function(input)
					if colorpicker or darknesss then
						if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
							cp(input.Position)
						end
					end
